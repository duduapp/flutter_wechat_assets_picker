...
class CustomAudioPageBuilder extends StatefulWidget {
  const CustomAudioPageBuilder({
    Key key,
    this.asset,
    this.state,
  }) : super(key: key);
  ...
  @override
  State<StatefulWidget> createState() => _CustomAudioPageBuilderState();
}

class _CustomAudioPageBuilderState extends State<CustomAudioPageBuilder> {
  /// A [StreamController] for current position of the [_controller].
  /// 控制器当前的播放进度
  final StreamController<Duration> durationStreamController =
  StreamController<Duration>.broadcast();
