...
import '../custom_asset_picker_viewer.dart';
...
class _VideoPageBuilderState extends State<VideoPageBuilder> {
  ...
  Future<void> initializeVideoPlayerController() async {
    ...
    _controller = CachedVideoPlayerController.network(Uri.parse(url).toString());
    if (mounted) {
      setState(() {});
    }
    try {
...
class CustomVideoPageBuilder extends StatefulWidget {
  const CustomVideoPageBuilder({Key key, this.asset, this.state}) : super(key: key);
  ...
  /// [State] for asset picker viewer.
  /// 资源查看器的状态[State]
  final CustomAssetPickerViewerState state;

  @override
  _CustomVideoPageBuilderState createState() => _CustomVideoPageBuilderState();
}

class _CustomVideoPageBuilderState extends State<CustomVideoPageBuilder> {
  /// Controller for the video player.
  /// 视频播放的控制器
  CachedVideoPlayerController _controller;
