...
class CustomAssetPickerViewer extends StatefulWidget {
  const CustomAssetPickerViewer(
      {Key key,
  ...
  final List<dynamic> originList;

  final Function(dynamic originData) onEditClicked;
  ...
  @override
  CustomAssetPickerViewerState createState() => CustomAssetPickerViewerState();
  ...
  static Future<List<AssetEntity>> pushToViewer(
    ...
    try {
      final Widget viewer = CustomAssetPickerViewer(
...
class CustomAssetPickerViewerState extends State<CustomAssetPickerViewer>
  ...
  /// 当前资源的Getter
  AssetEntity get currentAsset => assets.elementAt(currentIndex);
  ...
  Widget assetPageBuilder(BuildContext context, int index) {
    ...
    switch (asset.type) {
      case AssetType.audio:
        builder = CustomAudioPageBuilder(asset: asset, state: this);
        break;
      case AssetType.image:
        builder = CustomImagePageBuilder(
          asset: asset,
          state: this,
      ...
      case AssetType.video:
        builder = CustomVideoPageBuilder(asset: asset, state: this);
        break;
          ...
          child: Row(
            children: <Widget>[
              ...
              if (widget.actionIcons != null) ...[
                IconButton(
                  icon: widget.actionIcons[0],
                  onPressed: () {
                    widget.originList?.removeAt(currentIndex);
                    if (assets.length == 1) {
                    ...
                    if (currentIndex == assets.length) {
                      pageStreamController.add(assets.length -1);
                      currentIndex--;
                    }
                    setState(() {
                ...
                IconButton(
                  icon: widget.actionIcons[1],
                  onPressed: () {
                    if (widget.onEditClicked != null) {
                      widget.onEditClicked(widget.originList[currentIndex]);
                    }
                  },
                )
              ],
        ...
        child: StreamBuilder<int>(
          ...
          builder: (BuildContext _, AsyncSnapshot<int> snapshot) {
            ...
            return GestureDetector(
              onTap: () {
                if (assets == widget.selectedAssets) {
                ...
                child: Selector<AssetPickerViewerProvider, List<AssetEntity>>(
                  ...
                  builder: (
                    ...
                    final AssetEntity asset =
                        assets.elementAt(snapshot.data);
                    final bool isSelected =
                        currentlySelectedAssets.contains(asset);
                    if (isAppleOS) {
